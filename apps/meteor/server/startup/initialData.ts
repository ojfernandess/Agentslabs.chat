...
Meteor.startup(async () => {
	...
	if (process.env.TEST_MODE === 'true') {
		...
		if (await Users.findOneByEmailAddress(adminUser.emails![0].address)) {
			throw new Meteor.Error(`Email ${adminUser.emails![0].address} already exists`, "Agents Labs Chat PRO can't run in test mode");
		}

		if (!(await checkUsernameAvailability(adminUser.username!))) {
			throw new Meteor.Error(`Username ${adminUser.username} already exists`, "Agents Labs Chat PRO can't run in test mode");
		}
