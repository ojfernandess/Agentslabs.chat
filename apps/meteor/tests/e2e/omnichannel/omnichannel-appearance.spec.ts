...
test.describe('OC - Livechat Appearance - CE', () => {
	...
	test.afterAll(async ({ api }) => {
		const res = await api.post('/settings/Livechat_title', { value: 'Agents Labs Chat PRO' });
	...
	test('OC - Livechat Appearance - Change Livechat Title', async ({ page }) => {
		await test.step('expect to have default value', async () => {
			await expect(poLivechatAppearance.inputLivechatTitle).toHaveValue('Agents Labs Chat PRO');
		});
